@extends('layouts.app')
@section('title', 'Edit Buku - Lentera Aksara')
...
<div class="relative bg-gradient-to-r from-[var(--color-accent)] to-[#2F4F4F] text-white rounded-xl mb-8 p-6 overflow-hidden">
    <div class="relative z-10">
        <h1 class="text-2xl sm:text-3xl font-serif mb-1">@t('Edit Buku')</h1>
        <p class="text-sm text-white/90">{{ $book->title }}</p>
    </div>
...
<div class="max-w-4xl mx-auto">
    <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden transition-colors duration-300">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="relative rounded-lg overflow-hidden border border-gray-200 dark:border-gray-600 bg-[#F5F5F5] dark:bg-gray-700 h-60 md:h-72 flex items-center justify-center">
                    <div id="coverPreview" class="w-full h-full flex items-center justify-center">
                        @if($book->cover)
                            <img id="existingCover" src="{{ asset('storage/' . $book->cover) }}" alt="Cover Buku" class="w-full h-full object-cover cover-img show">
                        @else
            ...
            <div class="md:col-span-2">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        ...
                        <input id="title" name="title" type="text" value="{{ old('title', $book->title) }}" placeholder="@t('Masukkan judul buku')"
                    ...
                    <div>
                        ...
                        <input id="author" name="author" type="text" value="{{ old('author', $book->author) }}" placeholder="@t('Nama penulis')"
                            class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-shadow" required>
                    </div>

                    <div>
                        <label for="isbn" class="block mb-2 text-sm font-medium text-[var(--color-text)] dark:text-gray-200">ISBN</label>
                        <input id="isbn" name="isbn" type="text" value="{{ old('isbn', $book->isbn) }}" placeholder="9783161484100"
                    ...
                    <div>
                        ...
                        <input id="price" name="price" type="number" min="0" value="{{ old('price', $book->price) }}" placeholder="150000"
                            class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-shadow">
                    </div>

                    <div>
                        <label for="stock" class="block mb-2 text-sm font-medium text-[var(--color-text)] dark:text-gray-200">@t('Stok')</label>
                        <input id="stock" name="stock" type="number" min="0" value="{{ old('stock', $book->stock) }}" placeholder="0"
                            class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-shadow">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="categories" name="categories[]" multiple class="w-full min-h-[120px] px-3 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent">
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ ($book->categories->contains($category->id) || collect(old('categories'))->contains($category->id)) ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">@t('Tahan Ctrl/Cmd untuk memilih beberapa kategori.')</p>
                    </div>

                    <div class="sm:col-span-2">
                        <label for="description" class="block mb-2 text-sm font-medium text-[var(--color-text)] dark:text-gray-200">@t('Deskripsi')</label>
                        <textarea id="description" name="description" rows="4" placeholder="@t('Deskripsi atau sinopsis buku')" class="w-full px-4 py-2 border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-[var(--color-accent)] focus:border-transparent transition-shadow">{{ old('description', $book->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-800/50 flex flex-col-reverse md:flex-row items-center justify-between gap-3">
            ...
            <p class="text-xs text-gray-500">@t('Pastikan semua informasi sudah benar sebelum menyimpan.')</p>
        </div>
    </form>
</div>
@endsection
...
function previewCover(event) {
    ...
    if (!file) {
        // If no file selected, keep existing cover or placeholder
        // If existing cover exists, keep it (do nothing)
        return;
    }
